...
func ClientIpFromRequest(ctx context.Context, privateNetworks []*net.IPNet, r *http.Request) (string, error) {
	...
	// finally, just fallback to the contexts of the X-Real-Ip header (which
	// could be empty)
	return realIp, nil
}

func isPrivateAddr(ctx context.Context, privateNetworks []*net.IPNet, addr string) (bool, error) {
	const op = "controller.isPrivateAddr"
	if privateNetworks == nil {
	...
	// This iterates forward from the client, through the IPs for the first
	// public addr...
	//
	// we may want to actually iterate backward from boundary until we find the
	// first public addr from what ever proxy/lb is in front of boundary.
...
func InitPrivateNetworks(ctx context.Context, cidrBlocks []string) error {
	const op = "controller.initPrivateNetworks"
	if len(cidrBlocks) == 0 {
